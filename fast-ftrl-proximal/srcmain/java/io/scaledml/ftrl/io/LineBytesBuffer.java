...
public class LineBytesBuffer {
    ...
    public static LineBytesBuffer buildFromString(String s) {
        return buildFromString(s, Charsets.US_ASCII);
    }

    public static LineBytesBuffer buildFromString(String s, java.nio.charset.Charset charset) {
        LineBytesBuffer line = new LineBytesBuffer();
        ReaderInputStream readerInputStream = new ReaderInputStream(new StringReader(s), charset);
        FastBufferedInputStream fastBufferedInputStream = new FastBufferedInputStream(readerInputStream);
        try {
            line.readLineFrom(fastBufferedInputStream);
            return line;
        } catch (IOException e) {
            // todo fix this lol
            return null;
